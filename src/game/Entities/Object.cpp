...
void WorldObject::GetValidPointInAngle(float &x, float &y, float &z, float dist, float angle, bool meAsSourcePos, bool ignoreLOSOffset, float allowHeightDifference) const
{
	...
	Position dest;
	dest.x = x + dist * cos(angle);
	dest.y = y + dist * sin(angle);

	TerrainInfo const* _map = GetTerrain();
	float ground = _map->GetHeightStatic(dest.x, dest.y, MAX_HEIGHT, true);
	float floor = _map->GetHeightStatic(dest.x, dest.y, z, true);
	dest.z = fabs(ground - z) <= fabs(floor - z) ? ground : floor;

	// collision occurred
	bool result = false;
	if (ignoreLOSOffset)
		result = VMAP::VMapFactory::createOrGetVMapManager()->getObjectHitPos(GetMapId(), x, y, z + 0.5f, dest.x, dest.y, dest.z + 1.0f, dest.x, dest.y, dest.z, -0.5f);
	else
		result = VMAP::VMapFactory::createOrGetVMapManager()->getObjectHitPos(GetMapId(), x, y, z + 0.5f, dest.x, dest.y, dest.z + 2.0f, dest.x, dest.y, dest.z, -0.5f);
	...
	if (result)
	{
		...
		dist = sqrt((x - dest.x)*(x - dest.x) + (y - dest.y)*(y - dest.y));
	}
	...
	for (int j = 0; j < 10; ++j)
	{
		...
		if (fabs(z - dest.z) > allowHeightDifference)
		{
			dest.x -= step * cos(angle);
			dest.y -= step * sin(angle);
			ground = _map->GetHeightStatic(dest.x, dest.y, MAX_HEIGHT, true);
			floor = _map->GetHeightStatic(dest.x, dest.y, z, true);
			dest.z = fabs(ground - z) <= fabs(floor - z) ? ground : floor;
		}
		else
		{
